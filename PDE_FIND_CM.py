...
# # Funkcije za deriviranje podataka:
...
def TikhonovDiff(f, dx, lam, d = 1):
    ...
    where A is trapezoidal integration and D is finite differences for first derivative
...
def ConvSmoother(x, p, sigma): #convolution smoother
    """
...
# # Funkcije specifične za PDE-FIND:
...
def build_linear_system(u, dt, dx, D = 3, P = 3,time_diff = 'poly',space_diff = 'poly',lam_t = None,lam_x = None, width_x = None,width_t = None, deg_x = 5,deg_t = None,sigma = 2):
    ...
    Input:
        ...
        Optional:
            ...
            sigma = standard deviation of gaussian smoother
                    ...
                    default = 2
    Output:
        utt = column vector of length u.size
        R = matrix with ((D+1)*(P+1)) of column, each as large as utt
    ...
    ########################
    # First take the second time derivative for the left hand side of the equation
    ########################
    
    utt = np.zeros((n2,m2), dtype=np.complex64)
    ...
    if time_diff == 'FDconv':
        ...
        for i in range(n2):
            utt[i,:] = FiniteDiff(Usmooth[i + offset_x,:],dt,2)

    elif time_diff == 'poly':
        T= np.linspace(0,(m-1)*dt,m)
        for i in range(n2):
            utt[i,:] = PolyDiff(u[i+offset_x,:],T,diff=2,width=width_t,deg=deg_t)[:,1]

    elif time_diff == 'Tik':
        for i in range(n2):
            utt[i,:] = TikhonovDiff(u[i + offset_x,:], dt, lam_t, d=2)

    else:
        for i in range(n2):
            utt[i,:] = FiniteDiff(u[i + offset_x,:],dt,2)
    
    utt = np.reshape(utt, (n2*m2,1), order='F')
    ...
    for d in range(D+1):
        ...
        for p in range(P+1):
            ...
            if d > 0: rhs_description[d*(P+1)+p] = rhs_description[d*(P+1)+p]+                                                   'u_{' + ''.join(['x' for _ in range(d)]) + '}'

    return utt, Theta, rhs_description


# In[9]:


def print_pde(w, rhs_description, utt = 'u_tt'):
    pde = utt + ' = '
    first = True
...
# # Funkcije specifične za rijetke modele:

# In[10]:


def TrainSTRidge(R, Utt, lam, d_tol, maxit = 25, STR_iters = 10, l0_penalty = None, normalize = 2, split = 0.8, print_best_tol = False):
    """
